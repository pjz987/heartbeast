...
func set_size(size := 16) -> Inventory:
	_item_boxes.clear()
...
func get_item_boxes() -> Array[ItemBox]:
	return _item_boxes
...
func remove_item(item: Item, amount: int = 1) -> Inventory:
	var item_box_index = _find_item_box_index_with_item(item)
	if item_box_index != -1:
		_change_item_amount(item_box_index, -amount)
	return self
...
func has_item(search_item) -> bool:
...
func _find_item_box_with_item(search_item: Item) -> ItemBox:
	...
	# We're only concerned with the first box that contains the item we're looking for
