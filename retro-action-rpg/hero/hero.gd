...
@onready var roll_state = HeroRollState.new().set_actor(self) as HeroRollState
@onready var weapon_state = HeroWeaponState.new().set_actor(self) as HeroWeaponState
...
func take_hit(other_hitbox: Hitbox) -> void:
	...
	Events.request_camera_screenshake.emit(4, 0.0) # setting amount to 0.0 so I don't get nauseous
